...
void loop()
{
    ...
    for (int i = life; i > 0; i--)
        wbpro_blitBuf8(4, 75, 240, 239 - i * 16, 0, 16, 21, (uint8_t *)sprites); //wbpro_blitBuf8(int x:我們把最右側的生命圖示顯示出來後就能自動往左增加剩餘生命)
...
void PlayerMovement()
{
    wbpro_blitBuf8(4, 75, 240, masterX, 299, 16, 21, (uint8_t *)sprites); //將主角顯示出來
...
void EnemyCtrl()
{
    ...
    for (int i; i < 10; i++)
    {
        ...
        {
            enemyY[i] += 3; //設定向下飛的速度
            wbpro_blitBuf8(3, 150, 240, enemyX[i], enemyY[i], 11, 11, (uint8_t *)sprites);
    ...
    for (int i = 0; i < 100; i++)
    {
        if (bulletAlive[i])
        {
            ...
            case 1: //顯示與移動
                ...
                //////////
                // data 從哪邊抓資料！
                wbpro_blitBuf8(11, 70, 240, bulletX[i], bulletY[i], 2, 5, (uint8_t *)sprites);
                if (bulletY[i] < 0)
                {
                    bulletStatus[i]++;
                }
